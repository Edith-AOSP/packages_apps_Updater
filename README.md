...
Edith Updater downloads, verifies, and installs full OTA packages. The codebase retains its
LineageOS ancestry, but its package identity, product properties, server contract, storage,
branding, and optional Edith services are defined for Edith.

## Edith integration contract

The application is a platform-signed privileged `system_ext` app with package name
`com.edith.ota`. Add the `Updater` module to the product packages. Its Soong definition pulls in
the following required modules automatically:

- `default-permissions_com.edith.ota`, which grants the runtime notification permission by
  default;
- `init.edith-updater.rc`, which creates `/data/system_updates` as `system:cache` with mode
  `0770` and required file-based encryption.

The privileged permissions that require allowlisting are declared in `app/com.edith.ota.xml`.
...
exported packages use the public `Edith updates/` directory.

The surrounding Edith sepolicy must retain this file-context mapping (currently provided by
...
It must also retain the `seapp_contexts` rule that assigns platform-signed `com.edith.ota` to
the `updater_app` domain. Edith inherits that domain's update_engine, OTA-file, custom-property,
...
| --- | --- | --- |
| `ro.edith.device` | Edith device codename | OTA and changelog filenames |
| `ro.edith.version` | Installed Edith version / branch | Displayed build version and OTA/changelog URLs |
| `ro.edith.build.type` | Build type | OTA feed query |
...
All three Edith-specific properties must be non-empty in production. Missing device or branch
...
into DataStore. Room migrations accept both the old Edith version-1 table (without `type`) and
...
```text
https://project-edith.vercel.app/api/{branch}/device/{device}.json
```

`api/{branch}/device/{device}.json` uses the strict schema below. The legacy `response` wrapper and its
...
[
  {
    ...
    "files": [
      {
        "filename": "Edith_device-17.0-20260619-0000.zip",
        ...
        "size": 1926282058,
        "url": "https://sourceforge.net/projects/project-edith/files/device/Edith_device-17.0-20260619-0000.zip"
      }
    ],
    "type": "ci",
    "version": "17.0"
...
- `version`: non-blank Edith release version.
...
`tools/edith_feed.py` uses only the Python standard library. It reads authoritative values from
the OTA ZIP and computes size and SHA-256 itself:

```sh
tools/edith_feed.py generate-ota Edith_device-17.0-build.zip \
  --url https://sourceforge.net/projects/project-edith/files/device/Edith_device-17.0-build.zip \
  --version 17.0 \
  --output device.json

tools/edith_feed.py validate-ota device.json \
  --artifact Edith_device-17.0-build.zip
```
...
```text
https://project-edith.vercel.app/api/{branch}/changelogs/{device}.json
```
...
```sh
./push-update.sh Edith_device-17.0-build.zip [UNVERIFIED] [SERIAL]
```

The script accepts only `Edith_DEVICE-VERSION-*.zip`, verifies the connected
`ro.edith.device`, validates OTA metadata through `tools/edith_feed.py`, computes SHA-256, checks
