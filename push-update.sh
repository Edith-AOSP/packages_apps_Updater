...
usage() {
    ...
    echo "Push an Edith OTA ZIP to $updates_dir and register it with Updater."
    echo
    echo "Expected filename: Edith_DEVICE-VERSION-*.zip"
...
case "$zip_name" in
    Edith_*-*-*.zip) ;;
    *) die "Filename must match Edith_DEVICE-VERSION-*.zip" ;;
esac
...
name_remainder=${zip_name#Edith_}
...
temporary_directory=$(mktemp -d "${TMPDIR:-/tmp}/edith-updater.XXXXXX") || \
...
connected_device=$(adb_cmd shell getprop ro.edith.device | tr -d '\r')
if [ -n "$connected_device" ] && [ "$connected_device" != "$ota_device" ]; then
    die "OTA is for $ota_device, but connected device is $connected_device"
fi

build_type=$(adb_cmd shell getprop ro.edith.build.type | tr -d '\r')
[ -n "$build_type" ] || build_type=ci
case "$build_type" in *[!A-Za-z0-9._-]*) die "Invalid ro.edith.build.type" ;; esac

script_directory=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd -P)
python3 "$script_directory/tools/edith_feed.py" generate-ota "$zip_path" \
...
adb_cmd shell "test -d '$updates_dir'" || \
    die "$updates_dir does not exist; boot a build containing init.edith-updater.rc"
...
echo "Registered $zip_name with Edith Updater."
